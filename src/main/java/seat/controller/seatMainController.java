...
@RequestMapping(value=seatForm, method=RequestMethod.GET)
   public String seatForm(@RequestParam(value="mstart",required=false) String mstart, 
      ...
      PrintWriter writer = response.getWriter();
            ...
            //로그인 안했으면 로그인하고 다시 돌아와라
         if(session.getAttribute("loginInfo")==null) {
            writer.print("<script> alert('로그인이 필요한 작업입니다. 로그인 페이지로 이동합니다 '); </script>");
            writer.flush();
   ...
   public String seat(@RequestParam("checkedSeat") String checkedSeat,
      ...
      for(int i=0;i<sNo.length;i++) {
         ...
         System.out.println("선택한 sno/status : " + sno + "/" + status);
         
         if(Integer.parseInt(status)==1) { 
            writer.print("<script>alert('"+sno+"이미 예약된 좌석입니다')</script>");
            writer.flush();
            System.out.println("1");
            model.addAttribute("scode",scode); //theater 쓽 t_seat, seat 쓽 scode
            model.addAttribute("premium",premium); //theater 쓽 t_seat, seat 쓽 scode
            model.addAttribute("selectRg",selectRg); 
            model.addAttribute("timeTableRecord",timeTableRecord);
            return getPage;
         }
